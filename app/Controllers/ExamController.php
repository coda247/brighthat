...
class ExamController
{
    ...
    {
        $this->question = new ExamQuestion();
        $this->answer = new ExamAnswer();
    ...
    public function CreateExamSession(Request $request, Response $response)
    {
         ...
         if($this->validator->failed())
       {
        ...
        $exam_type = $this->getRequest($request,'exam_type');
        $questions = $this->startExamSession($exam_id,$user_id,$subscription_id,$exam_type);
        return $this->customResponse->is200Response($response,$questions);
    }
    ...
    public function closeSession($session_id)
    {
        ...
        $studentScore = $this->studentScore($session_id,$sessionInfo->data->exam_id);
    ...
    public function startExamSession($exam_id,$user_id,$subscription_id,$exam_type){
        $obj = new stdClass();$exam_session_id = uniqid();
        $subInfo = $this->creditController->subscriptionInfo($subscription_id);
        $checkSub = $this->creditController->checkSub($user_id);
        if ($checkSub->status == 'success') {
            $checkLimit  = $this->examSession->where(["user_id"=>$user_id,"subscription_id"=>$subscription_id])->count();
            if ($checkLimit < $subInfo->packInfo->exams) {
                try {
                ...
                $obj->status = 'error';
                $obj->numbr_of_exams = $subInfo->packInfo->exams;
                $obj->message = "Number of free exams reached, buy another credit pack to take exam";
                return $obj;
            } 
        }else{
            $obj->status = 'error';
            $obj->message = "Sorry, you do not have an active subscription, buy a credit pack to take exam";
            return $obj;
        }
    }
